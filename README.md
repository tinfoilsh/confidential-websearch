...
| ------------------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TINFOIL_API_KEY`               | -                                | Tinfoil API key for the in-enclave safeguard model and the privacy filter enclave                                                                                                  |
| `PII_ENCLAVE`                   | -                                | Privacy filter enclave hostname (e.g. `pii-filter.tinfoil.sh`) used for PII checking. Required in production.                                                                      |
...
| `max_results`           | int      | no       | `8`          | Number of results to return. Values from 1 to 30 are accepted; `0` uses the default and negative values are rejected.                                                                                                                       |
...
```json
{
  "pages": [{ "url": "string", "content": "string (text)" }],
  "results": [
    {
      "url": "string",
